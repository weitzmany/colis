...
4. **Sets Up Local Domain**: Automatically configures `.local` domain with Caddy reverse proxy (automatic)
5. **Configures IDE Colors**: Sets up unique project colors with branch-based themes (automatic)
6. **Initializes Git Workflow**: Automatically sets up git configuration, hooks, templates (mandatory)
7. **Validates Setup**: Verifies all files were copied correctly
8. **Single Command**: One command (`npx @colis/rig init`) does everything
9. **Smart Filtering**: Automatically excludes "local" commands/rules meant only for the packages repo
10. **Lifecycle Management**: Provides ongoing verify, repair, and refit capabilities
...
7. **Lifecycle Management**: Provide ongoing verify, repair, and refit capabilities
...
Beyond initial project setup, Project Initialization provides ongoing lifecycle management through verify, repair, and refit commands.
