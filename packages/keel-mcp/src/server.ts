...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
  tools: [
    ...
    },
    {
      name: 'report_issue',
      description:
        'File a bug report on the @colis packages GitHub repo (weitzmany/colis). The issue is attributed to the AI expert who found it. Always call list_issues first to avoid duplicates.',
      inputSchema: {
        type: 'object',
        properties: {
          package: {
    ...
    },
    {
      ...
      description:
        'Create a feature request on the @colis packages GitHub repo (weitzmany/colis). Attributed to the AI expert requesting it.',
      inputSchema: {
        type: 'object',
        properties: {
          package: {
    ...
    },
    {
      name: 'list_issues',
      description:
        'List open issues on the @colis packages GitHub repo. Filter by package, type, or status. Call this before filing a new issue to check for duplicates.',
      inputSchema: {
        type: 'object',
        properties: {
          package: {
    ...
    },
    {
      name: 'close_issue',
      description:
        'Close a @colis GitHub issue after confirming a fix works. Only the original opener (consuming project) should call this. Optionally add a closing comment.',
      inputSchema: {
        type: 'object',
        properties: {
          issue_number: {
  ...
  try {
    ...
    switch (name) {
      ...
      case 'get_docs_links':
        ...
        break;
      case 'report_issue':
        result = reportIssue(input as unknown as Parameters<typeof reportIssue>[0]);
        break;
      case 'request_feature':
        result = requestFeature(input as unknown as Parameters<typeof requestFeature>[0]);
        break;
      case 'list_issues':
        result = listIssues(input as Parameters<typeof listIssues>[0]);
        break;
      case 'close_issue':
        result = closeIssue(input as unknown as Parameters<typeof closeIssue>[0]);
        break;
      default:
        return {
