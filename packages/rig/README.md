...
**First time using the rig package?** Link it globally once, then use it in any project:
...
```bash
npm init -y
npm install file:../../packages/packages/rig
npx @colis/rig init
```
...
1. **Already linked globally** — The package is ready to use. Just run `npm link @colis/rig` in any project.

2. **Always initialize** — After linking, run `npx @colis/rig init` to set everything up correctly.
...
The rig package provides several foundational features that help streamline development workflows. Each feature is designed to be easy to use and well-documented.
...
npx @colis/rig port-manager init

# Get a port assigned to your project
npx @colis/rig port-manager allocate --project-name my-project --app-type nextjs
...
**Supercharge your IDE** — The rig package includes Cursor IDE rules and commands that automatically enhance your development workflow. Everything is copied to your project during initialization.
...
2. 🚀 **Try it out**: Run `npx @colis/rig init` in a test project
