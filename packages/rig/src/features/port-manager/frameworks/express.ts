/**
 * Express/Node.js Framework Handler
 */
...
export class ExpressHandler implements FrameworkHandler {
  getName(): string {
    return 'express';
  }
  ...
  async detect(projectPath: string): Promise<boolean> {
    ...
    if (await fs.pathExists(packageJsonPath)) {
      ...
      return (
        packageJson.dependencies?.express !== undefined ||
        packageJson.devDependencies?.express !== undefined
      );
    }
    return false;
  }
