...
export async function initializeProject(options: InitOptions = {}): Promise<InitResult> {
  ...
  try {
    ...
    if (!options.skipPortManager) {
      ...
      try {
        const portManagerResult = await portManagerInit({
        ...
        result.domainSetup = portManagerResult.domainSetup;
        ...
        if (result.domainSetup) {
          console.log(chalk.dim(`    → Local domain configured: ${result.domainSetup.domain}`));
        }
      } catch (error: unknown) {
        // Clear the loading indicator
        process.stdout.write('\r' + ' '.repeat(40) + '\r');
