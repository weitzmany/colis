...
- `packages/core/src/features/port-manager/core/allocator.ts` - Updated ranges
- `packages/core/scripts/clean-port-registry.ts` - New cleanup script
- `packages/core/package.json` - Added cleanup command
...
```bash
cd packages/core && npm run clean:port-registry
```
