  ...
  async hasEntry(domain: string): Promise<boolean> {
    try {
      ...
      return lines.some(line => {
        ...
        // Match exact domain (not substring)
        // Format: "127.0.0.1 domain.local" or "127.0.0.1\tdomain.local"
        const match = trimmed.match(/^127\.0\.0\.1[\s\t]+(.+)$/);
        if (match && match[1]) {
          const hostDomain = match[1].trim();
