...
async function isGhInstalled() {
    try {
        await (0, execa_1.execa)('gh', ['--version']);
        return true;
    }
    catch {
        return false;
...
async function installGh() {
    ...
    try {
        if (platform === 'darwin') {
            await (0, execa_1.execa)('brew', ['install', 'gh']);
            return { installed: true, alreadyInstalled: false, method: 'brew' };
        }
        else if (platform === 'linux') {
            try {
                await (0, execa_1.execa)('sudo', ['apt', 'install', '-y', 'gh']);
            ...
            try {
                await (0, execa_1.execa)('winget', ['install', '--id', 'GitHub.cli']);
                return { installed: true, alreadyInstalled: false, method: 'winget' };
            }
            catch {
                try {
                    await (0, execa_1.execa)('choco', ['install', 'gh', '-y']);
...
async function checkGhAuth() {
    try {
        const { stdout } = await (0, execa_1.execa)('gh', ['auth', 'status']);
...
async function ghAuthLogin() {
    try {
        await (0, execa_1.execa)('gh', ['auth', 'login', '--web'], { stdio: 'inherit' });
        return checkGhAuth();
    }
    catch (error) {
        return {
...
async function checkRepoExists(repoSlug) {
    try {
        await (0, execa_1.execa)('gh', ['repo', 'view', repoSlug]);
        return true;
    }
    catch {
        return false;
...
async function setRepoVisibility(repoSlug, visibility) {
    try {
        await (0, execa_1.execa)('gh', ['repo', 'edit', repoSlug, `--visibility=${visibility}`]);
        return { success: true };
    }
    catch (error) {
        return {
...
async function createGhRepo(repoName, options) {
    try {
        ...
        if (exists) {
            const { stdout } = await (0, execa_1.execa)('gh', ['repo', 'view', repoSlug, '--json', 'url', '-q', '.url']);
