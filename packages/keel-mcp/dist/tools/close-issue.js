...
export function closeIssue(input) {
    ...
    try {
        ...
        if (comment) {
            execSync(`gh issue comment ${issue_number} --repo "${REPO}" --body "${comment.replace(/"/g, '\\"').replace(/\n/g, '\\n')}"`, { encoding: 'utf8', timeout: 15000 });
        }
        execSync(`gh issue close ${issue_number} --repo "${REPO}"`, { encoding: 'utf8', timeout: 15000 });
        return {
            success: true,
            issue_url: issueUrl,
