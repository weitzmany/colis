...
async function initializeProject(options: any) {
    ...
    if (!config.skipInit) {
      ...
      try {
        ...
        if (!initResult.success) {
          ...
          initResult.errors.forEach((error: string) => console.error(chalk.red(`  - ${error}`)));
          if (initResult.warnings.length > 0) {
            console.warn(chalk.yellow('\nWarnings:'));
            initResult.warnings.forEach((warning: string) => console.warn(chalk.yellow(`  - ${warning}`)));
          }
        ...
        if (initResult.warnings.length > 0) {
          ...
          const portManagerWarnings = initResult.warnings.filter((w: string) => w.includes('Port Manager'));
