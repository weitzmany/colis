import type { ApplyResult, LandfallInstallOptions } from '../types';
import { createApplyResult, writeFileIfAllowed } from '../utils/file-utils';

function renderLandfallWorkflow(options: LandfallInstallOptions): string {
...
export async function installLandfall(options: LandfallInstallOptions): Promise<ApplyResult> {
  const result = createApplyResult();
  await writeFileIfAllowed(
    options.projectRoot,
    '.github/workflows/landfall-deploy.yml',
    renderLandfallWorkflow(options),
