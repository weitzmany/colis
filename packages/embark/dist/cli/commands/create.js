...
// Dynamic import wrapper for @colis/rig
async function initializeProject(options) {
    // @ts-ignore - Dynamic import to avoid build-time dependency
    const { initializeProject: init } = await import('@colis/rig/features/commissioning');
    return init(options);
}
 ...
 */
async function findCorePackage(projectPath) {
    const possiblePaths = [
 ...
 */
async function checkMissingRules(projectPath, corePackagePath) {
    const missing = [];
    try {
 ...
 */
async function checkMissingCommands(projectPath, corePackagePath) {
    const missing = [];
    try {
 ...
 */
async function addFrameworkToProject(projectPath, options) {
    console.log(chalk.blue('🔍 Detecting existing tech stack...'));
    const techStack = await detectTechStack(projectPath);
    // Show detected tech
    console.log(chalk.green('\n✓ Current tech stack:'));
    ...
    if (await fs.pathExists(rootReadmePath)) {
        ...
        if (readme.includes('## Project Structure')) {
            readme = readme.replace(/(## Project Structure\n\n)/, `$1${frameworkInfo}`);
        }
        else {
            readme += `\n## Project Structure\n\n${frameworkInfo}`;
        }
...
export async function createCommand(options = {}) {
    try {
            ...
            if (frontend !== 'none' && backend !== 'none') {
                ...
                try {
                    ...
                    if (fullStackTemplate) {
                        ...
                        if (frontend)
                            frontendTemplate = await templateRegistry.getTemplate(frontend);
                        if (backend)
                            backendTemplate = await templateRegistry.getTemplate(backend);
        ...
        if (!useMultipleTemplates) {
            ...
            if (frontendTemplate)
                console.log(chalk.blue(`  Frontend: ${frontendTemplate.name}`));
            if (backendTemplate)
                console.log(chalk.blue(`  Backend: ${backendTemplate.name}`));
        }
